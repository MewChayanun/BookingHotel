...
import 'package:hotelbooking/widgets/custom_dialog.dart';
...
class Helper {
  ...
  static String getDateText(DateText dateText) {
    ...
    return "0${dateText.startDate} ${DateFormat('MMM', languageType.toString().split(".")[1]).format(DateTime.now())} - 0${dateText.endDate} ${DateFormat('MMM', languageType.toString().split(".")[1]).format(DateTime.now().add(Duration(days: 2)))}";
  }
  ...
  static String getLastSearchDate(DateText dateText) {
    LanguageType languageType =
        applicationcontext == null
            ? LanguageType.en
            : applicationcontext!.read<ThemeProvider>().languageType;
    return "${dateText.startDate} - ${dateText.endDate} ${DateFormat('MMM', languageType.toString().split(".")[1]).format(DateTime.now().add(Duration(days: 2)))}";
  }
  ...
  static Widget ratingStar({double rating = 4.5}) {
    return RatingBarIndicator(
      rating: rating,
      itemBuilder:
          (context, index) => Icon(Icons.star, color: AppTheme.primaryColor),
      itemCount: 5,
